...
- `azure/ai/contentunderstanding/models/_patch.py` - Models (AnalyzeLROPoller, .value property, KeyFrameTimesMs, RecordMergePatchUpdate, ProcessingLocation.GLOBAL alias)
...
Similarly for `ET.register_namespace` and XML namespace operations in `_utils/model_base.py`:
```python
# Wrong:
                    ET.register_namespace(
                        prop_meta.get("prefix"), prop_meta.get("ns")
                    )  # pyright: ignore  <-- wrong line
...
- `azure/ai/contentunderstanding/_utils/model_base.py` (2 pyright: ignore comments in XML namespace handling)
...
| File | Pylint Rules to Suppress | Placement | Reason |
|------|--------------------------|-----------|--------|
| `models/_enums.py` | `enum-must-be-uppercase` | After copyright header | `GLOBALEnum = "global"` uses mixed case (Azure SDK convention) |
| `_validation.py` | `protected-access` | After copyright header (already generated) | Accesses `_config` on client class |
| `_utils/model_base.py` | `no-member`, `comparison-with-callable` | **Append** to existing `# pylint: disable=protected-access, broad-except` line after copyright header | Accesses `__mapping__` and compares callables |
| `_utils/serialization.py` | `protected-access`, `inconsistent-return-statements`, `unnecessary-lambda-assignment` | New line after copyright header | Various generated patterns |
| `_operations/_operations.py` | `name-too-long`, `inconsistent-return-statements` | New line after copyright header | Function names >40 chars, missing return expressions |

**How to add suppressions**: Place a **single** consolidated `# pylint: disable=...` comment **AFTER** the copyright header block. The comment **must NOT** appear before `# coding=utf-8` — doing so triggers `file-needs-copyright-header` errors.

**Important**: Some generated files have a `# pylint: disable=...` line **before** `# coding=utf-8` (e.g., `# pylint: disable=too-many-lines`). **Remove** that pre-copyright line and merge all rules into a single `# pylint: disable=...` line placed **after** the copyright header. Do NOT leave two separate pylint disable lines.

**Example** (single consolidated line after copyright):
```python
# coding=utf-8
# --------------------------------------------------------------------------
...
| 8 | Parameter order for analyze methods | `_patch.py` and `aio/_patch.py` | `begin_analyze` convenience: `inputs` (required), `model_deployments, processing_location` (no `content_type`); `begin_analyze_binary`: `input_range, content_type, processing_location` |
| 9 | `ProcessingLocation.GLOBAL` alias | `models/_patch.py` in `patch_sdk()` | `ProcessingLocation.GLOBAL = ProcessingLocation.GLOBALEnum` |
...
**Problem**: The generated `begin_analyze_binary` does not expose `content_type` as a keyword parameter with a default — it extracts `content_type` via `kwargs.pop("content_type")` internally, requiring callers to always pass it.

**Why this matters**: Users uploading binary files (PDFs, images, etc.) should have `application/octet-stream` as the default without needing to specify it explicitly.

**Fix**: Override `begin_analyze_binary` to expose `content_type` as a keyword parameter with the correct default:
...
**Note on `inputs` parameter**: As of commit `4e98679`, `inputs` is **required** (not optional) in the generated code. The convenience overload signature reflects this: `inputs: list[_models.AnalyzeInput]` (no `Optional`, no default value).

**Fix**: Override methods with correct parameter order:

```python
# begin_analyze (convenience overload - no content_type, inputs is required)
def begin_analyze(
    self,
    analyzer_id: str,
    *,
...
### Fix 9: ProcessingLocation.GLOBAL Alias

**File**: `azure/ai/contentunderstanding/models/_patch.py`

**Problem**: The TypeSpec Python emitter generates `GLOBALEnum` instead of `GLOBAL` for `ProcessingLocation` because `global` is a Python reserved keyword. The emitter appends `Enum` to avoid the keyword conflict, but this produces a non-standard member name.

**Why this matters**: Users expect `ProcessingLocation.GLOBAL` (consistent with the REST API value `"global"` and other language SDKs). `GLOBALEnum` is confusing and undiscoverable.

**Note**: The `# pylint: disable=enum-must-be-uppercase` suppression in `models/_enums.py` is also caused by this emitter issue — `GLOBALEnum` is mixed case. Both will be resolved when the emitter is fixed upstream.

**Fix**: Add a `GLOBAL` alias in `patch_sdk()`:
...
- `azure/ai/contentunderstanding/models/_patch.py` - Model customizations - **Fixes 2, 4, 5, 6, 9**
